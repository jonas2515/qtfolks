        ...
        int row)
{
    ...
    if(!name.firstName().isEmpty())
        qWarning() << tr("First name") << name.firstName() << noIcon << noAction;
    if(!name.lastName().isEmpty())
        qWarning() << tr("Last name") << name.lastName() << noIcon << noAction;
    ...
    if(!nickname.nickname().isEmpty() && nickname.nickname() != contact.detail<QContactDisplayLabel>().label())
        qWarning() << tr("Nickname") << nickname.nickname() << noIcon << noAction;

    // Favorite
    QContactFavorite favorite = contact.detail<QContactFavorite>();
    if(favorite.isFavorite())
        qWarning() << tr("Favorite") << tr("true") << noIcon << noAction;
    ...
    if (!genderString.isNull())
        qWarning() << tr("Gender") <<  genderString << noIcon << noAction;
    ...
    foreach(const QContactNote& note, notes) {
        ...
        if(!noteStr.isEmpty())
            qWarning() << tr("Note") << noteStr << noIcon << noAction;
    }
    ...
    if(!org.name().isEmpty()) {
        QString label(org.name() + QLatin1String(" (") + org.title() +
                QLatin1String(")"));
        qWarning() << tr("Organization") <<  label << noIcon << noAction;
    }
    ...
    foreach(const QContactPhoneNumber& phoneNumber, phoneNumbers) {
        ...
        if(!phoneStr.isEmpty())
            qWarning() << tr("Phone") << phoneStr << noIcon << noAction;
    }
    ...
    foreach(const QContactEmailAddress& emailAddress, emailAddresses) {
        ...
        if(!emailStr.isEmpty())
            qWarning() << tr("Email") << emailStr << noIcon << noAction;
    }
    ...
    if(!birthday.isEmpty()) {
        qWarning() << tr("Birthday") << birthday.date().toString() << noIcon
                << noAction;
    }
    ...
    if(!address.isEmpty()) {
        ...
        qWarning() << tr("Address") << label << noIcon << noAction;
    }
